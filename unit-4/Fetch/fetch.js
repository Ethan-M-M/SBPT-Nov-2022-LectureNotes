...
fetch(testEndpoint)
    ...
    !   JSON
        ...
        - Everything is wrapped in quotes with mninor exceptions (integers / booleans)
...
log(myText);
//log(JSON.parse(myText)); // changing the name value
...
The Process:
    ...
    Fetch differs from jQuery:
        - The promise returned won't reject on HTTP error status even if it's a 404 or 500 (internal / external server error)
*/
...
/* function getCatFact() {
    fetch(url)
        .then(r => r.json()) // taken response, json-ified it
        .then(d => log(d.data[0])); // taken data, logged it
}
*/
//* async/await version
